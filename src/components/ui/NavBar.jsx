...
export default function NavBar({ sectionRefs }) {
  ...
  const cta = useRef(null);//call to action button Lets Talk
  const tl = gsap.timeline();
  gsap.registerPlugin(ScrollTrigger);

  useEffect(() => {
    const lenis = new Lenis();//lenis for smooth scroll
    function raf(time) {
      lenis.raf(time);
      requestAnimationFrame(raf);//for scroll position
    }
    requestAnimationFrame(raf);
  });
  //Navbar move in from top
  useEffect(() => {
  ...
  return (
    ...
    >
      <a href="#hero" aria-label="main" className="z-50 text-secondary-600 font-black font-quarrel text-7xl">Prabhu</a>
      ...
      <nav className=" space-x-7 font-grotesk text-body-3 sm:block">
        ...
        <a href="#experience" className="group relative hidden md:inline-block">
